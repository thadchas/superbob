...
Usage: python bob_wxo_trigger.py <command> [options]
"""
...
class WXOAgentTrigger:
    ...
    def trigger_agent(self, agent_name: str, user_input: str, wait: bool = True, timeout: int = 120) -> Dict[str, Any]:
        ...
        Uses the chat completions endpoint for synchronous calls,
        or the runs endpoint for async polling.
        """
        try:
...
Usage:
  ...
  python bob_wxo_trigger.py trigger <agent_name> <message> [--no-wait] [--timeout=120]
  python bob_wxo_trigger.py status <run_id>

Examples:
  python bob_wxo_trigger.py list
  python bob_wxo_trigger.py trigger SuperBob_2079Hm "Search for AI news"
  python bob_wxo_trigger.py trigger SuperBob_2079Hm "Generate image" --no-wait
  python bob_wxo_trigger.py status abc123-run-id
""")
        sys.exit(1)
